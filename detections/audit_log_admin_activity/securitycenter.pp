locals {
  audit_log_admin_activity_security_command_center_detection_common_tags = merge(local.audit_log_admin_activity_detection_common_tags, {
    service = "GCP/SecurityCommandCenter"
  })

  audit_log_admin_activity_detect_disabled_security_command_center_sql_columns = replace(local.audit_log_admin_activity_detection_sql_columns, "__RESOURCE_SQL__", "resource_name")
}
...
benchmark "audit_logs_admin_activity_security_command_center_detections" {
  ...
  tags = merge(local.audit_log_admin_activity_security_command_center_detection_common_tags, {
    type = "Benchmark"
  })
}

detection "audit_log_admin_activity_detect_disabled_security_command_center" {
  title           = "Detect Disabled Security Command Center"
  ...
  severity        = "medium"
  query           = query.audit_log_admin_activity_detect_disabled_security_command_center
...
query "audit_log_admin_activity_detect_disabled_security_command_center" {
  sql = <<-EOQ
    select
      ${local.audit_log_admin_activity_detect_disabled_security_command_center_sql_columns}
    from
